...
{ 
  ...
  void ROSA_main::init(const std::shared_ptr<rclcpp::Node>& nh)
  {
    /* Params */
    input_pcd = nh->declare_parameter<std::string>("rosa_main/pcd", "null");
    Radius = nh->declare_parameter<double>("rosa_main/radius", -1.0);
    th_mah = nh->declare_parameter<double>("rosa_main/th_mah", -1.0);
    delta = nh->declare_parameter<double>("rosa_main/delta", -1.0);
    numiter_drosa = nh->declare_parameter<int>("rosa_main/num_drosa", -1);
    numiter_dcrosa = nh->declare_parameter<int>("rosa_main/num_dcrosa", -1);
    k_KNN = nh->declare_parameter<int>("rosa_main/k_KNN", -1);
    sample_radius = nh->declare_parameter<double>("rosa_main/sample_r", -1.0);
    alpha_recenter = nh->declare_parameter<double>("rosa_main/alpha", -1.0);
    pt_downsample_voxel_size = nh->declare_parameter<double>("rosa_main/pt_downsample_size", -1.0);
    ne_KNN = nh->declare_parameter<int>("rosa_main/estimation_number", -1);
    angle_upper = nh->declare_parameter<double>("rosa_main/upper_bound_angle_inner_decomp", -1.0);
    length_upper = nh->declare_parameter<double>("rosa_main/upper_bound_length_inner_decomp", -1.0);
    prune_flag = nh->declare_parameter<bool>("rosa_main/Prune", false);
    length_lower = nh->declare_parameter<double>("rosa_main/lower_bound_length", -1.0);
    prune_lower = nh->declare_parameter<double>("rosa_main/lower_bound_prune_angle", -1.0);
    ori_num = nh->declare_parameter<int>("rosa_main/upper_bound_original_points_num", -1);
    ground = nh->declare_parameter<bool>("rosa_main/Ground", false);
    estNum = nh->declare_parameter<int>("rosa_main/estimation_num", -1);

    /* Visualization */
    vis_utils_ = std::make_shared<PlanningVisualization>(nh);
    visFlag = true;
    
    // 创建定时器
    vis_timer_ = nh->create_wall_timer(
        std::chrono::duration<double>(0.1),
        std::bind(&ROSA_main::VisCallback, this)
    );
    ...
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[34m[SSD] Initialized!\033[34m");
  }
  ...
  void ROSA_main::main()
{
    ...
    // ROS2 logging
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[34m[SSD] SSD latency = %lf ms. Normal estimation costs %lf ms.\033[34m", rt, nrlt);
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[35m[SSD] --- <SSD finished> --- \033[35m");
}
  ...
  void ROSA_main::VisCallback()
{
    if (visFlag == true)
    {
        ...
        pcl::PointCloud<pcl::PointXYZ> pcloud, calcloud, drosa, cutplane;
        pcl::PointCloud<pcl::Normal> normals, orientations;
        pcl::PointXYZ pt_, cutpt_;
        pcl::Normal n_, o_;

        // 将 P.ori_pts_ 的点转换到 pcloud
        for (int i = 0; i < (int)P.ori_pts_->points.size(); ++i)
        {
            pt_.x = P.ori_pts_->points[i].x;
            pt_.y = P.ori_pts_->points[i].y;
        ...
        // 将 P.pts_ 的点转换到 calcloud，应用比例和中心偏移
        for (int j = 0; j < (int)P.pts_->points.size(); ++j)
        {
            pt_.x = P.pts_->points[j].x * P.scale + P.center(0);
            pt_.y = P.pts_->points[j].y * P.scale + P.center(1);
        ...
        // 将法向量存储到 normals 和 orientations
        for (int k = 0; k < (int)P.normals_->points.size(); ++k)
        {
            n_.normal_x = P.normals_->points[k].normal_x;
            n_.normal_y = P.normals_->points[k].normal_y;
        ...
        // 将 skeleton_ver_cloud 的点转换到 vertex_debug
        pcl::PointCloud<pcl::PointXYZ> vertex_debug;
        pcl::PointXYZ vertex_debug_pt;
        for (int i = 0; i < (int)skeleton_ver_cloud->size(); ++i)
        {
  ...
  void ROSA_main::pcloud_adj_matrix_mahalanobis(double& r_range)
{
    ...
    for (int i = 0; i < pcd_size_; ++i)
    {
        ...
        for (int j = 0; j < (int)indxs.size(); ++j)
        {
            ...
            weight1 = mahalanobis_leth(p1, v1, p2, v2, r_range);
            weight2 = mahalanobis_leth(p2, v2, p1, v1, r_range);
            weight = std::min(weight1, weight2);
    ...
    // ROS2中的日志输出
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] calculate adj matrix time = %lf ms.\033[32m", adj_time);
}
/******************************************************************************** */
  ...
  void ROSA_main::rosa_drosa()
{
    ...
    // ROS2日志输出
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] drosa estimation time = %lf ms.\033[32m", drosa_time);
}
  ...
  void ROSA_main::rosa_dcrosa()
{
    ...
    for (int n = 0; n < numiter_dcrosa; ++n)
    {
        ...
        for (int i = 0; i < (int)pset.rows(); ++i)
        {
            ...
            newpset2.row(i) = svd.matrixU().col(0).transpose() *
                              (svd.matrixU().col(0) * (pset.row(i) - local_mean.transpose())) +
                              local_mean.transpose();
        }
    ...
    std::chrono::duration<double, std::milli> dcrosa_ms = dcrosa_t2 - dcrosa_t1;
    double dcrosa_time = (double)dcrosa_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] dcrosa estimation time = %lf ms.\033[32m", dcrosa_time);
}
  ...
  void ROSA_main::rosa_lineextract()
{
    ...
    std::chrono::duration<double, std::milli> line_ms = line_t2 - line_t1;
    double line_time = (double)line_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] line extraction time = %lf ms.\033[32m", line_time);
}
  ...
  void ROSA_main::rosa_recenter()
{
    ...
    std::chrono::duration<double, std::milli> recenter_ms = recenter_t2 - recenter_t1;
    double recenter_time = (double)recenter_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] recentering time = %lf ms.\033[32m", recenter_time);
}
...
void ROSA_main::graph_decomposition()
{
    ...
    /* Tree + DFS + decomposition */
    std::vector<std::vector<int>>().swap(P.branches);
    P.visited.resize(P.graph.size(), false);
    ...
    std::chrono::duration<double, std::milli> graph_ms = graph_t2 - graph_t1;
    double graph_time = (double)graph_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] graph decomposition time = %lf ms.\033[32m", graph_time);
}
...
void ROSA_main::inner_decomposition()
{
    ...
    std::chrono::duration<double, std::milli> inner_ms = inner_t2 - inner_t1;
    double inner_time = (double)inner_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] inner decomposition time = %lf ms.\033[32m", inner_time);
}
  ...
  void ROSA_main::branch_merge()
{
    ...
    for (int i = 0; i < (int)P.branches.size(); ++i)
    {
        ...
        {
            P.visited[i] = true;
            std::cout << "2_element_Merge_id:" << i << std::endl;
            merge_one = merge_branch(P.branches[i]);
            temp_branches.push_back(merge_one);
            continue;
        }
        ...
        else
        {
            if (P.degrees(P.branches[i].front()) > 1 && P.degrees(P.branches[i].back()) > 1)
            {
                RCLCPP_WARN(rclcpp::get_logger("ssd_logger"), "Without Merging Intermediate Branch...");
                continue;
            }
            else
            {
                P.visited[i] = true;
                std::cout << "Merge_id:" << i << std::endl;
                merge_one = merge_branch(P.branches[i]);
                temp_branches.push_back(merge_one);
            }
        }
    ...
    std::vector<std::vector<int>> temp_branches_merge;
    for (int i = 0; i < (int)P.branches.size(); ++i)
    {
        if ((int)P.branches[i].size() == 2 && P.branches[i].front() == P.branches[i].back())
            continue;
    ...
    std::vector<int> seg_set;
    for (auto x : P.branches)
    {
        std::vector<int>().swap(seg_set);
        for (int i = 0; i < (int)x.size() - 1; ++i)
        {
            std::vector<int>().swap(indexer);
    ...
    std::cout << "branch_merge->segments: " << seg_count << std::endl;
    std::cout << "branch_merge->pairs: " << P.branch_seg_pairs.size() << std::endl;

    auto bm_t2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> bm_ms = bm_t2 - bm_t1;
    double bm_time = (double)bm_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] branch merge time = %lf ms.\033[32m", bm_time);
}
...
void ROSA_main::prune_branches()
{
    std::vector<std::vector<int>> updated_branches;
    bool prune_flag = false;
    for (auto x : P.branches)
    {
        if (x.size() == 2 && (P.degrees(x.front()) == 1 || P.degrees(x.back()) == 1))
    ...
    std::cout << "prune_branch->segments: " << seg_count << std::endl;
    std::cout << "prune_branch->pairs: " << P.branch_seg_pairs.size() << std::endl;
}
  ...
  void ROSA_main::restore_scale()
{
    ...
    std::chrono::duration<double, std::milli> rs_ms = rs_t2 - rs_t1;
    double rs_time = (double)rs_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] restore scale time = %lf ms.\033[32m", rs_time);
}
...
void ROSA_main::cal_inner_dist()
{
    ...
    std::chrono::duration<double, std::milli> cid_ms = cid_t2 - cid_t1;
    double cid_time = (double)cid_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] calculate inner dist time = %lf ms.\033[32m", cid_time);
}
...
void ROSA_main::distribute_ori_cloud()
{
    ...
    /* Update sub_space_scale */
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>().swap(P.sub_space_scale);
    pcl::PointCloud<pcl::PointXYZ>::Ptr sub_space_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    for (auto set : P.branch_seg_pairs)
    {
        sub_space_cloud.reset(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    std::chrono::duration<double, std::milli> doc_ms = doc_t2 - doc_t1;
    double doc_time = (double)doc_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] distribute original cloud time = %lf ms.\033[32m", doc_time);
}
  ...
  void ROSA_main::storeRealGraph()
{
    ...
    std::vector<int> newVerticesIdxs;
    for (const auto& value : st)
        newVerticesIdxs.push_back(value);
    for (int k = 0; k < (int)newVerticesIdxs.size(); ++k)
        temp_vertices.row(k) = P.outputVertices.row(newVerticesIdxs[k]);
    ...
    std::chrono::duration<double, std::milli> srg_ms = srg_t2 - srg_t1;
    double srg_time = (double)srg_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] store real graph time = %lf ms.\033[32m", srg_time);
}
  ...
  void ROSA_main::normal_estimation()
{
    RCLCPP_ERROR(rclcpp::get_logger("ssd_logger"), "Normal Estimation...");
  ...
  void ROSA_main::normalize()
{
    ...
    th_mah = 0.1 * Radius; // Determination relationship of constructing graph
    delta = pt_downsample_voxel_size; // Keep plane thickness and downsample size equal

    RCLCPP_ERROR(rclcpp::get_logger("ssd_logger"), "Final downsample voxel size: %f, Cloud size: %d.", pt_downsample_voxel_size, (int)P.cloud_with_normals->points.size());
    ...
    std::chrono::duration<double, std::milli> norm_ms = norm_t2 - norm_t1;
    double norm_time = (double)norm_ms.count();
    RCLCPP_INFO(rclcpp::get_logger("ssd_logger"), "\033[32m[SSD] normalize time = %lf ms.\033[32m", norm_time);
}
