...
#include <rclcpp/rclcpp.hpp>  // 替换ros/ros.h
...
public:
  ...
  void initMap(rclcpp::Node::SharedPtr nh);  // 替换ros::NodeHandle为rclcpp::Node::SharedPtr
  void inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
                      const Eigen::Vector3d& camera_pos);
  void inputPredictionCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
                      const Eigen::Vector3d& viewpoint);
...
// ! /* hierarchical_coverage_planner */ --------------------------------------------
  ...
  void initHCMap(rclcpp::Node::SharedPtr nh, pcl::PointCloud<pcl::PointXYZ>::Ptr& model);  // 修改NodeHandle为rclcpp::Node::SharedPtr
  void InternalSpace(std::map<int, pcl::PointCloud<pcl::PointXYZ>::Ptr>& seg_cloud, Eigen::MatrixXd& vertices, std::map<int, std::vector<int>>& segments);
  void SetOcc(pcl::PointCloud<pcl::PointXYZ>::Ptr& OccModel);
  void OuterCheck(std::vector<Eigen::VectorXd>& outers);
  std::vector<Eigen::Vector3d> points_in_plane(pcl::PointCloud<pcl::PointXYZ>::Ptr& point_cloud, Eigen::Vector3d& point_on_plane, Eigen::Vector3d& plane_normal, double thickness);
  bool isInMap_hc(const Eigen::Vector3d& pos);
  bool isInMap_hc(const Eigen::Vector3i& idx);
  bool safety_check(Eigen::Vector3d& pos);
  bool safety_check(Eigen::Vector3i& id);
  ...
  int get_Internal(const Eigen::Vector3d& pos);
  int get_Internal(const Eigen::Vector3i& id);
  void publishMap();  // 如果你要发布地图，在ROS2中需要用rclcpp::Publisher

  std::unique_ptr<HCMapParam> hcmp_;
  std::unique_ptr<HCMapData> hcmd_;
  std::unique_ptr<RayCaster> internal_cast_;
...
private:
  ...
  // 替换ros::Publisher为rclcpp::Publisher，并明确指定消息类型
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr occ_pub;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr free_pub;
