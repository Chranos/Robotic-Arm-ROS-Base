...
void SDFMap::initMap(rclcpp::Node::SharedPtr node) {
  ...
  node->declare_parameter("sdf_map.map_size_z", -1.0);
  node->get_parameter("sdf_map.map_size_z", z_size);
  ...
  node->declare_parameter("sdf_map.local_bound_inflate", 1.0);
  node->get_parameter("sdf_map.local_bound_inflate", mp_->local_bound_inflate_);

  node->declare_parameter("sdf_map.local_map_margin", 1);
  node->get_parameter("sdf_map.local_map_margin", mp_->local_map_margin_);

  node->declare_parameter("sdf_map.ground_height", 1.0);
  node->get_parameter("sdf_map.ground_height", mp_->ground_height_);

  node->declare_parameter("sdf_map.default_dist", 5.0);
  node->get_parameter("sdf_map.default_dist", mp_->default_dist_);
  ...
  node->declare_parameter("sdf_map.prediction_max_ray_length", -0.1);
  node->get_parameter("sdf_map.prediction_max_ray_length", mp_->max_ray_length_prediction);

  node->declare_parameter("sdf_map.virtual_ceil_height", -0.1);
  node->get_parameter("sdf_map.virtual_ceil_height", mp_->virtual_ceil_height_);

  node->declare_parameter("sdf_map.view_pos_threshold", -0.1);
  node->get_parameter("sdf_map.view_pos_threshold", mp_->view_thre_);
  ...
  mp_->min_occupancy_log_ = logit(mp_->p_occ_);
  mp_->unknown_flag_ = 0.01;
  std::cout << "hit: " << mp_->prob_hit_log_ << ", miss: " << mp_->prob_miss_log_
            << ", min: " << mp_->clamp_min_log_ << ", max: " << mp_->clamp_max_log_
            << ", thresh: " << mp_->min_occupancy_log_ << std::endl;
  ...
  md_->occupancy_buffer_pred_ = std::vector<char>(buffer_size, 0);
  // recon_poses_idx_ = std::vector<int>(buffer_size, 0);
  // recon_states_ = std::vector<int>(buffer_size, 0);
  ...
  // Try retrieving bounding box of map, set box to map size if not specified
  std::vector<std::string> axis = { "x", "y", "z" };
  for (int i = 0; i < 3; ++i) {
    node->declare_parameter("sdf_map.box_min_" + axis[i], mp_->map_min_boundary_[i]);
    node->get_parameter("sdf_map.box_min_" + axis[i], mp_->box_mind_[i]);

    node->declare_parameter("sdf_map.box_max_" + axis[i], mp_->map_max_boundary_[i]);
    node->get_parameter("sdf_map.box_max_" + axis[i], mp_->box_maxd_[i]);
  }
  posToIndex(mp_->box_mind_, mp_->box_min_);
  posToIndex(mp_->box_maxd_, mp_->box_max_);
...
void SDFMap::initHCMap(rclcpp::Node::SharedPtr node, pcl::PointCloud<pcl::PointXYZ>::Ptr& model)
{
  hcmp_.reset(new HCMapParam);
  hcmd_.reset(new HCMapData);
  mr_.reset(new MapROS);

  node->declare_parameter("hcmap.resolution", -1.0);
  node->get_parameter("hcmap.resolution", hcmp_->resolution_);
  node->declare_parameter("hcmap.interval", -1.0);
  node->get_parameter("hcmap.interval", hcmp_->proj_interval);
  node->declare_parameter("hcmap.plane_thickness", -1.0);
  node->get_parameter("hcmap.plane_thickness", hcmp_->thickness);
  node->declare_parameter("hcmap.checkScale", -1.0);
  node->get_parameter("hcmap.checkScale", hcmp_->checkScale);
  node->declare_parameter("hcmap.checkSize", -1);
  node->get_parameter("hcmap.checkSize", checkSize);
  node->declare_parameter("hcmap.inflateVoxel", -1);
  node->get_parameter("hcmap.inflateVoxel", inflate_num);
  node->declare_parameter("viewpoint_manager.visible_range", -1.0);
  node->get_parameter("viewpoint_manager.visible_range", hcmp_->size_inflate);
  node->declare_parameter("viewpoint_manager.zGround", false);
  node->get_parameter("viewpoint_manager.zGround", zFlag);
  node->declare_parameter("viewpoint_manager.GroundPos", -1.0);
  node->get_parameter("viewpoint_manager.GroundPos", zPos);

  occ_pub = node->create_publisher<sensor_msgs::msg::PointCloud2>("/hcmap/occ_map",10);
  free_pub = node->create_publisher<sensor_msgs::msg::PointCloud2>("/hcmap/free_map",10);
  
  /* find scene size */
  pcl::PointXYZ pt_min;
  pcl::PointXYZ pt_max;
  pcl::getMinMax3D(*model, pt_min, pt_max);
  // RCLCPP_INFO(node->get_logger(), "Map Minimum point: x=%f, y=%f, z=%f", pt_min.x, pt_min.y, pt_min.z);
  // RCLCPP_INFO(node->get_logger(), "Map Maximum point: x=%f, y=%f, z=%f", pt_max.x, pt_max.y, pt_max.z);
  double x_size, y_size, z_size;
  x_size = 4*hcmp_->size_inflate + pt_max.x - pt_min.x;
  y_size = 4*hcmp_->size_inflate + pt_max.y - pt_min.y;
  z_size = 4*hcmp_->size_inflate + pt_max.z - pt_min.z;
  ...
  int buffer_size = hcmp_->map_voxel_num_(0) * hcmp_->map_voxel_num_(1) * hcmp_->map_voxel_num_(2);
  hcmd_->occupancy_buffer_hc_ = std::vector<char>(buffer_size, 0);
  hcmd_->occupancy_inflate_buffer_hc_ = std::vector<char>(buffer_size, 0);
  hcmd_->occupancy_buffer_internal_ = std::vector<char>(buffer_size, 0);
...
void SDFMap::publishMap()
{
    ...
    cloud_msg.header.stamp = ros_clock.now();

    occ_pub->publish(cloud_msg);  // 在ROS2中使用->发布消息
    ...
    free_pub->publish(cloud_msg);  // 在ROS2中使用->发布消息
}
