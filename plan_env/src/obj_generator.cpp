...
/* ---------- initialize ---------- */
  ...
  for (int i = 0; i < obj_num; ++i) {
      auto pose_pub =
          node->create_publisher<geometry_msgs::msg::PoseStamped>("/dynamic/pose_" + std::to_string(i), 10);
      pose_pubs.push_back(pose_pub);
}

  // 使用rclcpp::Duration代替std::chrono
  auto update_timer = node->create_wall_timer(rclcpp::Duration::from_seconds(1 / 30.0), updateCallback);

  std::cout << "[dynamic]: initialize with " + std::to_string(obj_num) + " moving obj." << std::endl;
  ...
  time_update = node->now();  // rclcpp::Time替换ros::Time
  time_change = node->now();

  /* ---------- start loop ---------- */
  rclcpp::spin(node);  // rclcpp::spin替换ros::spin
...
void updateCallback() {
    ...
    /* ---------- change input ---------- */
    double dtc = (time_now - time_change).seconds();  // 改为.seconds()
    if (dtc > _interval) {
        for (int i = 0; i < obj_num; ++i) {
            /* ---------- use vel input ---------- */
            double vx, vy, vz, yd;
    ...
    /* ---------- update obj state ---------- */
    double dt = (time_now - time_update).seconds();  // 改为.seconds()
    time_update = time_now;
    for (int i = 0; i < obj_num; ++i) {
        obj_models[i].update(dt);
        visualizeObj(i);
...
void visualizeObj(int id) {
    ...
    mk.header.frame_id = "world";
    rclcpp::Clock clock(RCL_SYSTEM_TIME);
    mk.header.stamp = clock.now();  // 替换 ros::Time::now()
    ...
    obj_pub->publish(mk);  // 使用 ROS2 发布

    /* ---------- pose ---------- */
    geometry_msgs::msg::PoseStamped pose;
    pose.header.frame_id = "world";
    rclcpp::Clock clock(RCL_SYSTEM_TIME);
    pose.header.stamp = clock.now();  // 添加时间戳
    pose.pose.position.x = pos(0), pose.pose.position.y = pos(1), pose.pose.position.z = pos(2);
    pose.pose.orientation.w = 1.0;

    pose_pubs[id]->publish(pose);  // 使用 ROS2 发布
}
