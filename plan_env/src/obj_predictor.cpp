...
void ObjHistory::poseCallback(const nav_msgs::msg::Odometry::ConstSharedPtr& msg) {
    ++skip_;
    if (skip_ < ObjHistory::skip_num_) return;

    Eigen::Vector4d pos_t;
    pos_t(0) = msg->pose.pose.position.x;
    pos_t(1) = msg->pose.pose.position.y;
    pos_t(2) = msg->pose.pose.position.z;
    
    rclcpp::Clock clock(RCL_SYSTEM_TIME);  // 使用系统时钟
    pos_t(3) = (clock.now() - ObjHistory::global_start_time_).seconds();  // 使用Clock获取当前时间

    history_.push_back(pos_t);

    if ((int)history_.size() > queue_size_) {
        history_.pop_front();
    }
...
void ObjPredictor::init() {
  ...
  for (int i = 0; i < obj_num_; i++) {
    ...
    auto pose_sub = node_handle_->create_subscription<nav_msgs::msg::Odometry>(
  ...
  predict_timer_ = node_handle_->create_wall_timer(
    ...
    //rclcpp::Node::create_wall_timer 函数期望接收一个 std::chrono::duration 类型的持续时间参数， rclcpp::Duration，这是 ROS2 的一个内部类型，不符合模板匹配。
}
...
// void ObjPredictor::markerCallback(const visualization_msgs::msg::Marker::ConstSharedPtr& msg) {  // ROS2的消息类型
//   int idx = msg->id;
...
//   if (finish_num == obj_num_) {
//     marker_sub_.reset();  // 在ROS2中使用reset()替代shutdown()
//   }
// }
