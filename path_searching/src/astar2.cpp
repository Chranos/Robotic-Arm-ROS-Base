...
void Astar::init_hc(rclcpp::Node::SharedPtr node)
{
  // 声明并获取参数
  node->declare_parameter("astar/resolution_astar", -1.0);
  node->declare_parameter("astar/lambda_heu_hc", -1.0);
  node->declare_parameter("astar/max_search_time", -1.0);
  node->declare_parameter("astar/allocate_num", -1);
  node->declare_parameter("viewpoint_manager/zGround", false);
  node->declare_parameter("viewpoint_manager/GroundPos", -1.0);
  node->declare_parameter("viewpoint_manager/safeHeight", -1.0);

  // 获取参数值
  node->get_parameter("astar/resolution_astar", resolution_);
  node->get_parameter("astar/lambda_heu_hc", lambda_heu_hc_);
  node->get_parameter("astar/max_search_time", max_search_time_);
  node->get_parameter("astar/allocate_num", allocate_num_);
  node->get_parameter("viewpoint_manager/zGround", zFlag);
  node->get_parameter("viewpoint_manager/GroundPos", groundz);
  node->get_parameter("viewpoint_manager/safeHeight", safeheight);
  ...
  // Resize node pool and initialize nodes
  path_node_pool_.resize(allocate_num_);
  for (int i = 0; i < allocate_num_; i++) {
    path_node_pool_[i] = new Node;
  }
...
{ 
  ...
  // const auto t1 = ros::Time::now();
    ...
    // 使用 ROS2 Clock 获取当前时间
  rclcpp::Clock clock(RCL_SYSTEM_TIME);
  auto t1 = clock.now();  // 获取当前时间戳
  ...
  while (!open_set_.empty()) {
    ...
    if ((clock.now() - t1).seconds() > max_search_time_) {
