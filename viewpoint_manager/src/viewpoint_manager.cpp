...
namespace predrecon
{
  ...
  void ViewpointManager::init(rclcpp::Node::SharedPtr node)
{
    node_ = node;

    node_->get_parameter_or("viewpoint_manager/visible_range", visible_range, -1.0);
    node_->get_parameter_or("viewpoint_manager/viewpoints_distance", dist_vp, -1.0);
    node_->get_parameter_or("viewpoint_manager/fov_h", fov_h, -1.0);
    node_->get_parameter_or("viewpoint_manager/fov_w", fov_w, -1.0);
    node_->get_parameter_or("viewpoint_manager/pitch_upper", pitch_upper, -1.0);
    node_->get_parameter_or("viewpoint_manager/pitch_lower", pitch_lower, -1.0);
    node_->get_parameter_or("viewpoint_manager/zGround", zFlag, false);
    node_->get_parameter_or("viewpoint_manager/GroundPos", GroundZPos, -1.0);
    node_->get_parameter_or("viewpoint_manager/safeHeight", safeHeight, -1.0);
    node_->get_parameter_or("viewpoint_manager/safe_radius", safe_radius_vp, -1.0);
    node_->get_parameter_or("viewpoint_manager/attitude_type", attitude, std::string("null"));
    node_->get_parameter_or("viewpoint_manager/max_iter_num", max_iter_num, 0);
    node_->get_parameter_or("viewpoint_manager/pose_update", pose_update, true);
    ...
    fov_base = std::min(fov_h, fov_w) * M_PI / 360.0;

    percep_utils_.reset(new PerceptionUtils);
    percep_utils_->init(node);
    HCMap.reset(new SDFMap);

    RCLCPP_INFO(node_->get_logger(), "[ViewpointManager] Initialized!\033[34m");
}
  ...
  void ViewpointManager::setMapPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr input_map)
{
    pcl::PointCloud<pcl::PointXYZ>::Ptr new_model(new pcl::PointCloud<pcl::PointXYZ>);
    *new_model = *input_map;
    HCMap->initHCMap(node_, new_model);
    map_cloud_kdtree_.setInputCloud(new_model);
    raycaster_.reset(new RayCaster);
    raycaster_->setParams(HCMap->hcmp_->resolution_, HCMap->hcmp_->map_origin_);

    TS.map_flag_ = true;
    RCLCPP_WARN(node_->get_logger(), "[ViewpointManager] Input map successfully!! size = %ld", new_model->points.size());
...
void ViewpointManager::setModel(pcl::PointCloud<pcl::PointXYZ>::Ptr input_model)
{
    if (input_model->points.empty())
    {
        RCLCPP_ERROR(node_->get_logger(), "[ViewpointManager] Input model is empty!!");
        return;
    }
    pcl::PointCloud<pcl::PointXYZ>::Ptr new_model(new pcl::PointCloud<pcl::PointXYZ>);
    *new_model = *input_model;
    ...
    TS.model_flag_ = true;
    RCLCPP_WARN(node_->get_logger(), "[ViewpointManager] Input model successfully!! size = %ld", model_->points.size());
}
  ...
  void ViewpointManager::setNormals(std::map<Eigen::Vector3d, Eigen::Vector3d, Vector3dCompare0> &pt_normal_pairs)
{
    if (pt_normal_pairs.empty())
    {
        RCLCPP_ERROR(node_->get_logger(), "[ViewpointManager] Input normal Fail!! Input normal is empty!!!");
        return;
    }
    TS.normal_flag_ = true;
    MCI.pt_normal_pairs = pt_normal_pairs;
    RCLCPP_WARN(node_->get_logger(), "[ViewpointManager] Input model normals successfully!! size = %ld", MCI.pt_normal_pairs.size());
}
  ...
  void ViewpointManager::setInitViewpoints(pcl::PointCloud<pcl::PointNormal>::Ptr input_normal_vps)
{
    if (input_normal_vps->points.empty())
    {
        RCLCPP_ERROR(node_->get_logger(), "[ViewpointManager] Input viewpoints Fail!! Input viewpoint is empty!!!");
        return;
    }
    ...
    for (size_t i = 0; i < input_normal_vps->points.size(); ++i)
    {
        pcl::PointNormal nor_vp = input_normal_vps->points[i];
    ...
    TS.viewpoints_flag_ = true;
    RCLCPP_WARN(node_->get_logger(), "[ViewpointManager] Input viewpoints successfully!! size = %ld", VPI.vps_pose.rows());
}
  ...
  void ViewpointManager::updateViewpoints()
  {
    ...
    {
      RCLCPP_WARN(node_->get_logger(), "[ViewpointManager] No input viewpoints!! Use normals gengerate viewpoints!!");
    ...
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] primitive viewpoints library size = %d generation time = %lf ms.\033[32m", unc_vps_size, (double)lib_ms.count());

    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] ----------------------------------------------------------\033[32m");
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] number of updated viewpoints = %d.\033[32m", (int)VPP.final_vps_.size());
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] number of total voxels = %d.\033[32m", (int)MCI.cover_state.size());
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] number of seen voxels before iter updating = %d.\033[32m", sum_seen);
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] number of uncovered voxels before iter updating = %d.\033[32m", (int)uncoveredID.size());
    RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] ----------------------------------------------------------\033[32m");
    ...
    while ((int)uncoveredID.size() > 0 && unc_iter < iter_max && last_unc_num != (int)uncoveredID.size())
    {
      ...
      viewpointsPrune(VPI.vps_pose, temp_vps_voxcount);
      RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] Iter_%d -> number of updated viewpoints = %d.\033[32m", unc_iter, (int)VPP.final_vps_.size());
      ...
      RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] Iter_%d -> number of total voxels = %d.\033[32m", unc_iter, (int)MCI.cover_state.size());
      RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] Iter_%d -> number of seen voxels after updating = %d.\033[32m", unc_iter, sum_seen);
      RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] Iter_%d -> number of uncovered voxels after updating = %d.\033[32m", unc_iter, (int)uncoveredID.size());
      RCLCPP_INFO(node_->get_logger(),"\033[36m[ViewpointManager] ----------------------------------------------------------\033[32m");
  ...
  void ViewpointManager::viewpointsPrune(Eigen::MatrixXd vps_pose, vector<int> vps_voxcount)
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"No need prune viewpoints!");
      return;
    }
