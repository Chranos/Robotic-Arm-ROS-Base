...
void initUtils(rclcpp::Node::SharedPtr node)
{
    ...
    inputModel();
    viewpoint_manager_ = std::make_shared<ViewpointManager>();
    viewpoint_manager_->init(node);

    percep_utils_ = std::make_shared<PerceptionUtils>();
    percep_utils_->init(node);

    HCMap_ = std::make_shared<SDFMap>();
    HCMap_->initHCMap(node, model_);
    HCMap_->inputFreePointCloud(free_model_);

    raycast_ = std::make_shared<RayCaster>();
    raycast_->setParams(HCMap_->hcmp_->resolution_, HCMap_->hcmp_->map_origin_);
}
                    ...
                    const pcl::PointCloud<pcl::Normal>::Ptr &normals)
{
    ...
    for (int i = 0; i < (int)input_cloud->points.size(); ++i)
    {
        ...
        nm.header.frame_id = frame_id_;
        nm.header.stamp = rclcpp::Clock().now();  // Use ROS2 time
        nm.id = counter;
    ...
    pub->publish(pcloud_normals);  // Use ROS2 publisher method
}
                       ...
                       const double &scale, const Eigen::Vector4d &color)
{
    ...
    for (size_t i = 0; i < viewpoints->size(); ++i)
    {
        ...
        marker.header.frame_id = frame_id_;
        marker.header.stamp = rclcpp::Clock().now();  // Use ROS2 time
        marker.ns = "viewpoints";
        marker.id = i;
    ...
    pub->publish(marker_array);  // Use ROS2 publisher method
}
                ...
                const std::vector<std::vector<Eigen::Vector3d>> &list2)
{
    ...
    for (size_t j = 0; j < list1.size(); ++j)
    {
        ...
        mk.header.frame_id = "frame_id";  // Replace with actual frame_id variable
        ...
        mk.scale.y = 0.08f;
        mk.scale.z = 0.08f;
        mk.pose.orientation.w = 1.0f;

//         geometry_msgs::msg::Point pt;
//         for (size_t i = 0; i < list1[j].size(); ++i)
//         {
//             pt.x = list1      pt.y = list1 ;
//      .z = list1 ;
//             mpush_back(pt);
...
int main(int argc, char **argv)
{
    ...
    // rclcpp::Node::SharedPtr node = std::make_shared<rclcpp::Node>("viewpoint_manager_node");


    // nh->get_parameter_or("viewpoint_manager_node/cloud_path", model_path_, string("null"));
    // nh->get_parameter_or("viewpoint_manager_node/free_cloud_path", free_cloud_path_, string("null"));
    // nh->get_parameter_or("viewpoint_manager_node/downsampled_size", downsampled_size_, 0.1);
    // nh->get_parameter_or("viewpoint_manager_node/downsampled_size_for_viewpoint", downsampled_size_for_viewpoint_, 0.2);
    // nh->get_parameter_or("viewpoint_manager_node/min_box_z", min_box_z_, -1.0);
    // nh->get_parameter_or("viewpoint_manager_node/max_box_z", max_box_z_, -1.0);
    // nh->get_parameter_or("viewpoint_manager/viewpoints_distance", dist_vp_, -1.0);
    ...
    node->get_parameter_or("viewpoint_manager_node/cloud_path", model_path_, std::string("null"));
    node->get_parameter_or("viewpoint_manager_node/free_cloud_path", free_cloud_path_, std::string("null"));
    node->get_parameter_or("viewpoint_manager_node/downsampled_size", downsampled_size_, 0.1);
    node->get_parameter_or("viewpoint_manager_node/downsampled_size_for_viewpoint", downsampled_size_for_viewpoint_, 0.2);
    node->get_parameter_or("viewpoint_manager_node/min_box_z", min_box_z_, -1.0);
    node->get_parameter_or("viewpoint_manager_node/max_box_z", max_box_z_, -1.0);
    node->get_parameter_or("viewpoint_manager/viewpoints_distance", dist_vp_, -1.0);
    ...
    RCLCPP_INFO(node->get_logger(), "\033[33m[NormalViewpoint] all viewpoint quantity = %d. \033[32m", (int)updated_vps.size());
    RCLCPP_INFO(node->get_logger(), "\033[35m[NormalViewpoint] --- <Normal Viewpoint Generation finished> --- \033[35m");
    ...
    while (rclcpp::ok())
    {
        ...
        rclcpp::spin_some(node);  // Perform ROS2 callback processing
        loop_rate.sleep();
    }
