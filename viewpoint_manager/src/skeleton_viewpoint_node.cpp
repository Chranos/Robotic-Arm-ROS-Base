...
namespace predrecon
{
  ...
  void SkeletonViewpoint::init(rclcpp::Node::SharedPtr node)
{
    // * Module Initialization
    skeleton_operator.reset(new ROSA_main);
    HCMap.reset(new SDFMap);
    percep_utils_.reset(new PerceptionUtils);
    vis_utils_.reset(new PlanningVisualization(node));
    viewpoint_manager_.reset(new ViewpointManager);

    skeleton_operator->init(node);
    percep_utils_->init(node);
    viewpoint_manager_->init(node);

    // * Params Initialization
    node->get_parameter_or("hcplanner/mesh", mesh, std::string("null"));
    node->get_parameter_or("hcplanner/model_downsample_size", model_ds_size, -1.0);
    node->get_parameter_or("hcplanner/sample_step4normal", normal_step, -1.0);
    node->get_parameter_or("viewpoint_manager/viewpoints_distance", dist_vp, -1.0);
    node->get_parameter_or("viewpoint_manager/fov_h", fov_h, -1.0);
    node->get_parameter_or("viewpoint_manager/fov_w", fov_w, -1.0);
    node->get_parameter_or("viewpoint_manager/zGround", zFlag, false);
    node->get_parameter_or("viewpoint_manager/GroundPos", GroundZPos, -1.0);
    node->get_parameter_or("viewpoint_manager/safeHeight", safeHeight, -1.0);
    node->get_parameter_or("viewpoint_manager/safe_radius", safe_radius_vp, -1.0);
    node->get_parameter_or("hcplanner/fullcloud", fullcloud, std::string("null"));
    ...
    vis_timer_ = node->create_wall_timer(
        std::chrono::milliseconds(500), std::bind(&SkeletonViewpoint::visCallback, this));

    RCLCPP_INFO(node->get_logger(), "\033[35m[SkeletonViewpoint] Initialized! \033[32m");
  ...
  void SkeletonViewpoint::execution(rclcpp::Node::SharedPtr node)
  {
    ...
    // * Initialize Mapping
    HCMap->initHCMap(node, PR.occ_model);
    for (const auto &seg_cloud : skeleton_operator->P.seg_clouds_scale)
    {
        *PR.ori_model += *seg_cloud.second;
        ds.setInputCloud(seg_cloud.second);
    ...
    // ROS2 Logging
    RCLCPP_INFO(node->get_logger(), "\033[33m[SkeletonViewpoint] input points size = %d. \033[32m", (int)PR.model->points.size());
    ...
    RCLCPP_INFO(node->get_logger(), "\033[33m[SkeletonViewpoint] all viewpoint quantity = %d. \033[32m", viewpointNum);
    RCLCPP_INFO(node->get_logger(), "\033[35m[SkeletonViewpoint] --- <Skeleton Viewpoint Generation finished> --- \033[35m");



  }

  void SkeletonViewpoint::visCallback()
  {
...
int main(int argc, char **argv)
{
    ...
    auto nh = std::make_shared<rclcpp::Node>("skeleton_viewpoint_node");

    predrecon::SkeletonViewpoint skeleton_vp;
    skeleton_vp.init(nh);
    skeleton_vp.execution(nh);
   // test test;
    // ROS2 equivalent of ros::Duration(1.0).sleep();
    rclcpp::sleep_for(std::chrono::seconds(1));


    rclcpp::spin(nh); // 等待并处理所有消息

    return 0; // ROS2 会在程序结束时自动关闭
}
