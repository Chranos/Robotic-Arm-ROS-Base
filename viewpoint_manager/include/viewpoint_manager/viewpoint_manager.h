    ...
    struct TaskState
{
    ...
    bool isReady()  // 接受 logger 参数，以便打印日志
    {
        //rclcpp::Logger logger;
        if (!map_flag_)
        {
            RCLCPP_ERROR(rclcpp::get_logger("default"), "[ViewpointManager] Not ready: No input map!!");
            return false;
        }
        else if (!model_flag_)
        {
            RCLCPP_ERROR(rclcpp::get_logger("default"), "[ViewpointManager] Not ready: No input model!!");
            return false;
        }
        else if (!viewpoints_flag_)
        {
            RCLCPP_ERROR(rclcpp::get_logger("default"), "[ViewpointManager] Not ready: No input viewpoints!!");
            return false;
        }
        else if (!normal_flag_)
        {
            RCLCPP_ERROR(rclcpp::get_logger("default"), "[ViewpointManager] Not ready: No input normals!!");
            return false;
        }
        return true;
    }
...
public:
    ...
    void setModel(pcl::PointCloud<pcl::PointXYZ>::Ptr input_model);
    void setMapPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr input_map);
    void setNormals(std::map<Eigen::Vector3d, Eigen::Vector3d, Vector3dCompare0> &pt_normal_pairs);
    void setInitViewpoints(pcl::PointCloud<pcl::PointNormal>::Ptr input_normal_vps);
    void updateViewpoints();

    void getUpdatedViewpoints(std::vector<SingleViewpoint> &viewpoints);
    void getUncoveredModelCloud(pcl::PointCloud<pcl::PointXYZ> &cloud);
  ...
  private:
    ...
    void viewpointsPrune(Eigen::MatrixXd sub_pose, std::vector<int> sub_voxcount);
    void updatePoseGravitation(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, std::vector<int> &inner_ids, const int &cur_idx);
    void updateViewpointInfo(Eigen::VectorXd &pose, const int &vp_id);

    bool viewpointSafetyCheck(pcl::PointXYZ &vp);
    bool nearCheck(Eigen::Vector3d &cur_position, int &vox_num);
    std::vector<pcl::PointNormal> uncNeighVps(pcl::PointXYZ oript, pcl::Normal normal);
    ...
    /* Param */
    rclcpp::Node::SharedPtr node_;  // 使用 ROS2 中的 SharedPtr 节点
    double visible_range;
    double pitch_upper, pitch_lower;
    double fov_h, fov_w;
    bool zFlag;
    ...
    /* Utils */
    std::shared_ptr<SDFMap> HCMap;
    std::unique_ptr<RayCaster> raycaster_;
    std::unique_ptr<PerceptionUtils> percep_utils_;
