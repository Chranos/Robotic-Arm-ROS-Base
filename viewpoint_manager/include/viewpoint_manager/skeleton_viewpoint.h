...
#include <rclcpp/rclcpp.hpp>  // ROS2头文件
...
public:
  ...
  void execution(rclcpp::Node::SharedPtr node);
  /* Data */
  ViewpointResult PR;
  /* Utils */
  std::unique_ptr<ROSA_main> skeleton_operator;
...
private:
  ...
  std::string fullcloud;
  /* Data */
  std::string mesh;
  pcl::PointCloud<pcl::PointXYZ> uncovered_area;
  std::vector<Eigen::VectorXd> valid_viewpoints;
  std::vector<double> pitch_set;
  std::vector<bool> seg_visited_buffer_;
  ...
  /* Utils */
  std::shared_ptr<PlanningVisualization> vis_utils_;
  ...
  void visCallback();  // ROS2 TimerEvent
  ...
  /* Timer */
  rclcpp::TimerBase::SharedPtr vis_timer_;  // ROS2 Timer
